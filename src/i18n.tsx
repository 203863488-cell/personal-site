...
const projectTranslations: Record<"en", Record<string, LocalizedProjectFields>> = {
  en: {
    "isolated-sampling-board": {
      ...
      designPoints: ["Voltage-channel logic: a high-resistance divider first scales P-to-GND voltage, then AMC1301 provides isolation; TLV9062 conditions the isolated output around a 1.65V midpoint, with the PCB formula V(P->GND) ≈ 62.60 × (VADC_V - 1.65)", "Current-channel logic: a 20mΩ shunt is placed between P and N, so 4A peak current produces about 80mV; this signal passes through 39Ω input resistors, filtering, AMC1301 isolation, and op-amp conditioning, with the PCB formula I(P->N) ≈ 3.049 × (VADC_I - 1.65)", "Isolation logic: the voltage and current channels each use AMC1301, with ISO_5V_1 and ISO_5V_2 supplying the isolated input sides; the control side only receives conditioned ADC_V and ADC_I signals, reducing MCU exposure to power-ground noise and high-voltage risk", "Biasing logic: a 1.65V reference is generated from 3.3V, and TLV9062 conditions the output around that midpoint so a unipolar ADC can read bidirectional signal movement", "Debug logic: the board reserves TP points, 5V test points, voltage/current sensing ports, and ADC outputs; bring-up should verify supply and bias first, zero-input output second, and known voltage/current calibration last"],
      progress: "The schematic and PCB are complete. The board organizes voltage sensing, current sensing, 5V input, 3.3V output, ISO_5V_1, ISO_5V_2, AGND/GND, voltage/current ADC outputs, and test points into a directly debuggable hardware module. The page currently records the design logic; real board renders, schematic screenshots, measured waveforms, and calibration tables can be added once image files are available.",
      validation: ["Check 5V input, 3.3V regulation, and ISO_5V_1/ISO_5V_2 isolated supplies during startup", "With zero input, verify ADC_V and ADC_I remain near 1.65V to confirm the bias network and TLV9062 conditioning path", "Test the voltage channel at known DC voltage points and back-calculate with V(P->GND) ≈ 62.60 × (VADC_V - 1.65)", "Test the current channel with known load current and back-calculate with I(P->N) ≈ 3.049 × (VADC_I - 1.65)", "Use an oscilloscope to compare the isolated input side, AMC1301 output side, and ADC-conditioned output side to identify whether noise mainly comes from the power loop, isolation amplifier, or op-amp stage"],
      improvements: ["Add real PCB render and schematic screenshots as project images", "Add 0-85V voltage calibration tables, 0-4A current calibration tables, and linearity error curves", "Record ADC output ripple under PWM switching noise to evaluate whether the filter values need adjustment", "Document wiring notes, test-point usage, and competition on-site troubleshooting flow"]
    },
