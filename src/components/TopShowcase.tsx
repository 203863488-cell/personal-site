...
function MetricPanel({ metrics }: { metrics: PortfolioMetric[] }) {
  return (
    <div className="rounded-2xl border border-white/60 bg-white/58 p-5 shadow-[0_18px_55px_rgba(31,41,51,0.12)] backdrop-blur-md transition duration-300 hover:-translate-y-1 hover:bg-white/70">
          ...
          <div className="w-full">
            ...
            <div className="absolute bottom-10 left-1/2 flex w-[min(100%-2.5rem,80rem)] -translate-x-1/2 items-center justify-between gap-4">
              ...
              <div className="hidden gap-3 sm:flex">
                <button type="button" onClick={goToPrevious} className="secondary-button h-11 w-11 px-0 py-0" aria-label="Previous">
                  ←
                </button>
                <button type="button" onClick={goToNext} className="secondary-button h-11 w-11 px-0 py-0" aria-label="Next">
                  →
                </button>
              </div>
            </div>
