...
export function MetricTile({ badge, caption, className = "", icon: Icon, label, tone = "light", value }: MetricTileProps) {
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          {badge ? (
            <span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[0.62rem] font-semibold tracking-[0.08em] ${
              dark ? "bg-white/10 text-white/72" : "bg-[#EEF5FC] text-[#2563EB]"
            }`}>
              {badge}
            </span>
          ) : null}
