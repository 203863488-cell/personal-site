...
export function FeatureSlideVisual({ slide, language }: FeatureSlideVisualProps) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      {slide.imageUrl && <img src={slide.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover" />}
      <div className={`absolute inset-0 ${overlay}`} />
      {!isWelcome && <MetricCards metrics={slide.content[language].metrics} variant="floating" />}
    </div>
  );
}
