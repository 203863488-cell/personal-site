...
const personalProjectData: PortfolioProject[] = [
  {
    ...
    validation: [
      ...
      "功率因数：实测 PF 约 0.94，输入电流谐波表现得到改善。",
      "软启动：检查 NTC 限流和继电器旁路时序，避免上电浪涌直接冲击整流桥和母线电容。",
      "保护与热设计：完成关键器件应力、温升关注点和高压安全间距核对。"
    ],
    improvements: [
      "形成多负载点效率、PF、THD 和主要谐波的测试方法与记录模板。",
  ...
  },
  {
    ...
    responsibilities: [
      ...
      "评估 SiC MOSFET Qg、Coss、驱动电压、开关损耗和 dv/dt 特性，采用 UCC23513 与 +18V / -3V 双极性驱动。",
    ...
    designPoints: [
      ...
      "SiC 双极性栅压用于抑制误导通；布局优先保证栅极回路、Kelvin 源极和驱动退耦。",
    ...
    improvements: [
      "形成 25%、50%、75%、100% 负载效率、纹波、SR 波形和温升测试矩阵。",
      "沉淀频率限幅、软启动斜率、PI 参数、保护阈值和故障复位条件。",
      "完成启动、关断、负载阶跃、母线扰动、短时过载和长期运行测试流程。",
      "整理变压器规格、LCR 实测和 PCB 布局复盘资料。"
    ]
  },
  {
    id: "flyback",
    ...
    designPoints: [
      ...
      "220VAC 满载结果与宽输入设计、EMI/安规措施共同构成完整的方案与验证证据。"
    ],
    progress: "已完成原理图、磁件规格、PCB 样机、满载输出、效率、纹波和 MOSFET 应力测试。实测结果为 220VAC 输入、24V / 3A 输出、效率约 85%、纹波约 390mVPP、负载调整率 0.67%。",
    validation: [
      "220VAC 输入、24V / 3A 输出工况下完成满载运行。",
      "满载效率约 85%，与前期功率级和磁件估算目标一致。",
      "输出纹波约 390mVPP，负载调整率约 0.67%。",
      "通过 VDS 波形核对 MOSFET 应力和 RCD 吸收效果。",
      "宽输入能力：完成 85-265VAC 输入范围设计核对，并以 220VAC 满载结果作为主要实测证据。"
    ],
    improvements: [
      "形成 85VAC、110VAC、220VAC、265VAC 效率、纹波、VDS、VDD 和温升测试矩阵。",
      "沉淀 265VAC 高线下 MOSFET 应力、RCD 温升、整流管温升和变压器漏感核对方法。",
      "完成负载阶跃、短路/过载、轻载跳频和长期满载运行测试流程。",
      "形成保险丝、NTC、MOV、共模电感、Y 电容、EMI 和安规爬电距离设计记录。"
    ]
  },
  {
    ...
    designPoints: [
      ...
      "同时导出结构化 CSV 和原始日志，分别服务数据分析与通信链路复盘。"
    ],
    ...
    improvements: [
      "形成固件发送帧率与上位机接收率对比统计。",
      "支持导出样机编号、输入电压、负载、磁件批次、参数版本和测试备注。",
      "保留参数写入扩展接口，并定义安全状态门控、范围限制、二次确认和回读校验机制。",
      "沉淀真实串口长时间运行和 CSV 复盘样例。"
    ]
  },
  {
